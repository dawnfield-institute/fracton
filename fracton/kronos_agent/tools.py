...
class KronosTools:
    ...
    async def scan_repo(
        ...
        """
        Scan a repository and create a knowledge graph.
        ...
        for pattern in file_patterns:
            for file_path in repo.rglob(pattern):
                ...
                try:
                    ...
                    # Create node with SOURCE LOCATION metadata
                    # This is key - nodes are pointers to sources, not full content
                    rel_path = file_path.relative_to(repo)
                    
                    # Store summary/description, not full content
                    # The metadata points to the actual source
                    line_count = content.count('\n') + 1
                    preview = content[:500] + "..." if len(content) > 500 else content
                    
                    node_id = await self._memory.store(
                        content=preview,  # Just a preview for embedding
                        graph=graph_name,
                        node_type=node_type,
                        metadata={
                            # Source location - the key insight
                            "source_type": "file",
